...
def parse_creditcard_pdf(pdf_path, closing_date):
    """
    Handles two Wells Fargo credit card formats:
    Format A (Signify): 'Transaction Details' section, no card prefix
    Format B (Signature/FA): 'Transactions' with Payments/Credits/Purchases subsections, card prefix
    """
    transactions = []
    account_number = None
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page in pdf.pages:
            ...
            if not account_number:
                ...
                if not account_number:
                    ending_match = re.search(r'Account ending in (\d{4})', t)
                    if ending_match:
                        account_number = ending_match.group(1)
    ...
    if 'Transaction Details' in full_text:
        transactions = _parse_format_a(full_text, closing_date)
    else:
        transactions = _parse_format_b(full_text, closing_date)

    return transactions, account_number, ending_balance, balance_date


def _parse_format_a(full_text, closing_date):
    """Signify format: no card prefix, single amount column."""
    transactions = []
...
def _parse_format_b(full_text, closing_date):
    """Signature/FA format: card prefix, Payments/Credits/Purchases subsections."""
    transactions = []
    lines = full_text.split('\n')
    in_payments = in_credits = in_purchases = False

    for line in lines:
        line = line.strip()
        if not line or line.startswith('&'):
            continue
        if re.match(r'^(TOTAL |Fees Charged|Interest Charged|Page \d+|Continued)', line, re.I):
            in_payments = in_credits = in_purchases = False
            continue
        if re.match(r'^Payments$', line, re.I):
            in_payments, in_credits, in_purchases = True, False, False
            continue
        if re.match(r'^Other Credits$', line, re.I):
            in_payments, in_credits, in_purchases = False, True, False
            continue
        if re.match(r'^Purchases', line, re.I):
            in_payments, in_credits, in_purchases = False, False, True
            continue
        if not (in_payments or in_credits or in_purchases):
            continue

        # Try with card number prefix first
        tx = re.match(
            r'^(\d{4})\s+(\d{1,2}/\d{2})\s+(\d{1,2}/\d{2})\s+(\S{8,})\s+(.+?)\s+([\d,]+\.\d{2})\s*$',
            line
        )
        if tx:
            _, _, post_date_str, ref_num, description, amount_str = tx.groups()
        else:
            # Payments often have no card prefix
            tx = re.match(
                r'^(\d{1,2}/\d{2})\s+(\d{1,2}/\d{2})\s+(\S{8,})\s+(.+?)\s+([\d,]+\.\d{2})\s*$',
                line
            )
            if tx:
                _, post_date_str, ref_num, description, amount_str = tx.groups()
            else:
                continue
        ...
        year = infer_year(post_month, closing_date)
        post_date = datetime(year, post_month, post_day)
        signed = amount if in_purchases else -amount
...
def main():
    ...
    pdf_files = sorted(folder.glob('*WellsFargo.pdf'))
    if not pdf_files:
